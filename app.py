    ...
    if request.method == "POST":
        ...
        try:
            ...
            text_columns = [
               "Assessment_Rubic",
               "Strengths",
               "Areas_for_Improvment",
               "Suggested_corrective_measures"
        ]
            ...
            # Find student
            student_idx = df[df["Roll_Number"] == roll_number].index
            ...
            if len(student_idx) == 0:
                ...
                python_marks = float(request.form.get("python"))
                dwdm = float(request.form.get("dw"))
                dbms = float(request.form.get("dbms"))

                total_ca3 = ai + ml + python_marks + dwdm + dbms
                ...
                # Save data in CSV
                df.at[student_idx, "Artificial_Intelligence"] = ai
                df.at[student_idx, "Machine_Learning"] = ml
                df.at[student_idx, "Python_Programming"] = python_marks
                df.at[student_idx, "Data_Warehousing_and_Data_Mining"] = dwdm
                df.at[student_idx, "DBMS"] = dbms
                df.at[student_idx, "Total CA3 marks"] = total_ca3

                df.at[student_idx, "Assessment_Rubic"] = rubric
                df.at[student_idx, "Strengths"] = strengths
                df.at[student_idx, "Areas_for_Improvment"] = areas
